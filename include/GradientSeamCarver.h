...
public:
    GradientSeamCarver();
    cinder::Surface getGradientImage(cinder::Surface imgData);
    cinder::Surface computeVerticalSeam(cinder::Surface imgData);
    cinder::Surface getVerticalSeams(cinder::Surface imgData, int nSeams = 1);
    cinder::Surface getHorizontalSeams(cinder::Surface imgData, int nSeams = 1);
...
cinder::Surface GradientSeamCarver::getGradientImage(cinder::Surface imgData)
{
    Mat src, src_gray;
...
cinder::Surface GradientSeamCarver::getVerticalSeams(cinder::Surface imgData, int nSeams)
{
    timer->start();
    int w = imgData.getWidth();
    ImageSeam* verticalSeam_bt = getBottomToTop(imgData);
    ImageSeam* verticalSeam_tb = getTopToBottom(imgData);
    ImageSeam* verticalSeam = new ImageSeam[2*w];
    for(int i=0; i<w; i++){
        verticalSeam[2*i] = verticalSeam_bt[i];
        verticalSeam[2*i+1] = verticalSeam_tb[i];
    }
    std::vector<ImageSeam> c(verticalSeam, verticalSeam + 2*w);
...
cinder::Surface GradientSeamCarver::getHorizontalSeams(cinder::Surface imgData, int nSeams)
{
    timer->start();
    int h = imgData.getHeight();
...
cinder::Surface GradientSeamCarver::computeVerticalSeam(cinder::Surface imgData)
{
    Mat src, src_gray;
    ...
    int seamPath[w*h];
    ImageSeam* verticalSeam = new ImageSeam[w];
    ci::Surface s = cinder::Surface(cinder::fromOcv(grad));
    
    for(int x=0; x<w; x++){
        seamPath[x] = 0;
        verticalSeam[x] = ImageSeam();
        verticalSeam[x].index = x;
        verticalSeam[x].preValue = (s.getPixel(ci::Vec2i(x,0))).b;
        verticalSeam[x].totalValue = (s.getPixel(ci::Vec2i(x,0))).b;
    }
    ...
    for(int y=1; y<h; y++){
        for(int x=0; x<w; x++){
            ...
            if(x==0){//First Column
                if(verticalSeam[x].preValue <= verticalSeam[x+1].preValue){
                    verticalSeam[x].totalValue += (s.getPixel(ci::Vec2i(x,y))).b + verticalSeam[x].preValue;
                    seamPath[y*w+x] = 0;
                }
                else{
                    verticalSeam[x].totalValue += (s.getPixel(ci::Vec2i(x,y))).b + verticalSeam[x+1].preValue;
                    seamPath[y*w+x] = 1;
                }
    ...
    for (std::vector<ImageSeam>::iterator it=myvector.begin(); it!=myvector.begin() + 1; ++it)
    {
        ...
        //("\n(seam, value) = (%d,%u)",it->startingIndex,it->totalValue);
    }
    /*
     ci::Surface8u::Iter pixelIter = s.getIter();
     float testF = 0.0;
     unsigned int testI = 0;
     while( pixelIter.line() ) {
     while( pixelIter.pixel() ) {
     ci::Color color( pixelIter.r(), pixelIter.g(), pixelIter.b() );
     // printf("r=%f,g=%f,b=%f)",color.r,color.g,color.b);
     // printf("%#08x\n", rgb[pixelIter.y()*w + pixelIter.x()]);
     //printf("(x,y) = (%d,%d)\n",pixelIter.x(),pixelIter.y());
     testF += color.r;
     testI += rgb[pixelIter.y()*w + pixelIter.x()];
     ...
     printf("testF = %f", testF);
     printf("testI = %d", testI);
     */
    ...
    for(int x=w-2; x>=0; x--){
        for(int y=0; y<h; y++){
            ...
            if(y==0){//First Row
                ...
                if(verticalSeam[y-1].preValue <= verticalSeam[x].preValue && verticalSeam[y-1].preValue <= verticalSeam[y+1].preValue){
                    verticalSeam[y].totalValue += (imgData.getPixel(ci::Vec2i(x,y))).b + verticalSeam[y-1].preValue;
                    seamPath[y*w+x] = -1;
                }
                else if(verticalSeam[y].preValue <= verticalSeam[y+1].preValue){
